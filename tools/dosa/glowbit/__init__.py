...
class glowbitMatrix(glowbit):
    ...
    class graph1D(colourFunctions, colourMaps):
        ...
        def __init__(self, originX=0, originY=7, length=8, direction="Up", minValue=0, maxValue=255, colour=0xFFFFFF,
                     colourMap="Solid", update=False):
    ...
    def newGraph1D(self, originX=0, originY=7, length=8, direction="Up", minValue=0, maxValue=255, colour=0xFFFFFF,
                   colourMap="Solid", update=False):
    ...
    def updateGraph1D(self, graph, value):
        ...
        if graph.orientation == 1:
            ...
            for idxY in range(graph.originY, graph.originY + graph.inc * (graph.length), graph.inc):
                if n < N:
                    self.pixelSetXY(graph.originX, idxY,
                                    m(idxY, graph.originY, graph.originY + (graph.inc * graph.length - 1)))
                else:
        ...
        if graph.orientation == 0:
            ...
            for idxX in range(graph.originX, graph.originX + graph.inc * (graph.length), graph.inc):
                if n < N:
                    self.pixelSetXY(idxX, graph.originY,
                                    m(idxX, graph.originX, graph.originX + (graph.inc * graph.length - 1)))
                else:
    ...
    class graph2D(colourFunctions, colourMaps):
        ...
        def __init__(self, originX=0, originY=7, width=8, height=8, minValue=0, maxValue=255, colour=0xFFFFFF,
                     bgColour=0x000000, colourMap="Solid", update=False, bars=False):
            self.minValue = minValue
            self.maxValue = maxValue
            self.originX = originX
            self.originY = originY
    ...
    def updateGraph2D(self, graph, value):
        ...
        m = graph.colourMap
        self.drawRectangleFill(graph.originX, graph.originY - graph.height + 1, graph.originX + graph.width - 1,
                               graph.originY, graph.bgColour)
        for value in graph.data:
            ...
            if graph.bars == True:
                ...
                if x >= graph.originX and x < graph.originX + graph.width and y <= graph.originY and y > graph.originY - graph.height:
                    self.pixelSet(self.remap(x, y),
                                  m(y - graph.originY, graph.originY, graph.originY + graph.height - 1))
            x -= 1
        if graph.update == True:
            self.pixelsShow()
...
class stick(glowbit):
    ...
    class graph1D(colourFunctions, colourMaps):
        ...
        def __init__(self, minIndex=0, maxIndex=7, minValue=0, maxValue=255, colour=0xFFFFFF, colourMap="Solid",
                     update=False):
            self.minValue = minValue
            self.maxValue = maxValue
    ...
    def newGraph1D(self, minIndex=0, maxIndex=7, minValue=0, maxValue=255, colour=0xFFFFFF, colourMap="Solid",
                   update=False):
        ...
        while iters > 0:
            if iters % (self.numLEDs + 4) == 0:
                if iters % (2 * (self.numLEDs + 4)) == 0:
                    ...
                    self.addPulse(speed=-100, index=self.numLEDs, colourMap="Rainbow",
                                  colour=[-1, self.rgbColour(255, 255, 255), -1])
            self.pixelsFill(0)
            self.updatePulses()
            self.pixelsShow()
            iters -= 1
...
class matrix8x8(glowbitMatrix):
    ...
    def __init__(self, tileRows=1, tileCols=1, pin=18, brightness=20, mapFunction=None, rateLimitFPS=-1,
                 rateLimitCharactersPerSecond=-1, sm=0):
    ...
    def printTextWrap(self, string, x=0, y=0, colour=0xFFFFFF):
        ...
        for char in string:
            if Py < self.numLEDsY - 7:
                self.draw_char(char, Px, Py, colour)
            Px += 8
    ...
    def updateTextScroll(self):
        for textLine in self.scrollingTextList:
            ...
            for c in textLine.string:
                self.draw_char(c, -textLine.x + 8 * x, textLine.y, textLine.colour)
                x += 1
            textLine.x += 1
    ...
    @micropython.viper
    def draw_char(self, char, px: int, py: int, colour: int):
        """
        ...
        x = px
        y = py
        char_idx = (int(ord(char)) - 32) * 8
        n = int(self.numLEDs)
        max_col = int(min(8, int(self.numLEDsX) - px))
        if x < 0:
            min_col = -1 * x
            x = 0
        else:
        ...
        tile_cols = int(self.tileCols)
        for col in range(min_col, max_col):
            dat = int(petme128[char_idx + col])
    ...
    def update_rate_limit_cps(self, rate_limit_cps):
        """
        Changes the 8x8 matrix display's update rate in units of "characters of scrolling text per second".
        For example, a value of 2 would scroll 2 characters per second; leaving each character at least partly visible
        for 0.5 seconds.
        """
        self.rateLimit = rate_limit_cps * 8
